...
export default function AIResultsScreen() {
  ...
  const allResults: AIResult[] = state.aiResults ?? [];
  ...
  function deleteResult(id: string) {
    Alert.alert("Delete Result", "This cannot be undone.", [
      ...
      { text: "Delete", style: "destructive", onPress: () => dispatch({ type: "AI_RESULT_DELETE", resultId: id }) },
    ]);
  }

  function saveTitle() {
    if (editingId && editingTitle.trim()) {
      dispatch({ type: "AI_RESULT_UPDATE", resultId: editingId, payload: { title: editingTitle.trim() } });
    }
    setEditingId(null);
  }
  ...
  return (
    <ScreenContainer scroll>
      <Text style={s.title}>📁 AI Results</Text>
      <Text style={s.sub}>Reports, sheets and files saved by your AI assistant</Text>
      ...
      {filtered.length === 0 && (
        <View style={s.empty}>
          ...
          <Text style={s.emptySub}>Go to the AI Agent and ask it to create a report, spreadsheet, or PDF for you.</Text>
        </View>
      )}
...
const s = StyleSheet.create({
  ...
  cardMeta: { fontSize: FontSize.xs, color: Colors.textSecondary, lineHeight: 16 },
